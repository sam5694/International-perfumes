...
# Hardcoded subcategory URLs for Men's Fragrances
SUBCATEGORY_URLS = [
    "https://www.boutiqaat.com/ar-kw/men/fragrances/perfumes/l/",
    "https://www.boutiqaat.com/ar-kw/men/fragrances/hair-mist-1/l/",
    "https://www.boutiqaat.com/ar-kw/men/fragrances/perfume-gift-sets/l/",
    "https://www.boutiqaat.com/ar-kw/men/fragrances/refillable-fragrance-spray/l/",
    "https://www.boutiqaat.com/ar-kw/men/grooming/body-mist/l/",
]
...
class BoutiqaatDataPipeline:
    ...
    def run(self) -> bool:
        ...
        logger.info("=" * 80)
        logger.info("Starting Men's Fragrance Pipeline (Async – Semaphore=3)")
        logger.info("=" * 80)
        try:
    ...
    def _process_category(self, scraper: BoutiqaatScraper, category: dict) -> bool:
        ...
        try:
            ...
            if not products:
                logger.info(f"Skipping {category_name} - no products found")
                return True
    ...
    def _upload_product_image(self, product: Dict, category_name: str, subcategory_name: str) -> str:
        ...
        try:
            ...
            s3_path = (
                f"boutiqaat-data/year={datetime.now().strftime('%Y')}/month={datetime.now().strftime('%m')}/day={datetime.now().strftime('%d')}/men/fragrance/images/"
                f"{safe_category}"
            )
    ...
    def _upload_excel_file(self, local_path: str, category_name: str) -> bool:
        ...
        try:
            ...
            s3_path = (
                f"boutiqaat-data/year={datetime.now().strftime('%Y')}/month={datetime.now().strftime('%m')}/day={datetime.now().strftime('%d')}/men/fragrance/excel-files"
            )
